...
function drawMalevich(){
  ...
  push();
    rotate(5);
    fill(221, 152, 24);
    rect(.7 * width/2, .8 * height/4,(1 + spectrum[1]) * (.9 * width / 2),(1 + spectrum[1]) * (.9 * width / 2));
  pop();
  // rect 2, red over 1
  push();
    rotate(5);
    fill(173, 60, 43);
    rect(1.05 * width /3, .72 * height/2,(1 + spectrum[2]) * (.85 * width / 5), (1 + spectrum[2]) * (1.1 * height / 2));
  pop();
  // rect 3, green
  push();
    rotate(-2);
    fill(11, 66, 53);
    rect(1.2 * width / 2, .8 * height / 3, (1 + spectrum[3]) * (1.1 * width / 2), (1 + spectrum[3]) * (height / 4));
  pop();
  // rect 4, big black
  push();
    rotate(-1);
    fill(0);
    rect(3 * width / 4, height / 2, (1 + spectrum[4]) * (width / 18), (1 + spectrum[4]) * (.9 * height));
  pop();
  // rect 5, small black
  push();
    rotate(3);
    fill(0);
    rect(1.2 * width / 2, height / 3, (1 + spectrum[5]) * (width / 40), (1 + spectrum[5]) * (.9 * 3 * height / 4));
  pop();
  ...
  push();
    ...
    rotate(-10);
    fill(0);
    rect(0, 0, (1 + spectrum[6]) * (3 * width / 4), (1 + spectrum[6]) * (height / 100));
  pop();
}
